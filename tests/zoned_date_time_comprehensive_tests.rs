...
#[test]
fn from_time_offsetdatetime_and_into_roundtrip_for_zoned() {
    // Build a time::OffsetDateTime directly and convert to our ZonedDateTime and back
    ...
    // ZonedDateTime stores UTC-only semantics currently; assume UTC
    let todt = pdt.assume_utc();

    let ours: ZonedDateTime = todt.into();
...
fn time_based_arithmetic_hours_minutes_seconds_nanos_for_zoned() {
    ...
    assert_eq!(
        base0.plus_nanos(1_000_000_000),
        ZonedDateTime::of(LocalDateTime::of(2020, 1, 1, 0, 0, 1), ZoneId::UTC)
    );
    let prev = base0.minus_nanos(1);
    assert_eq!(
        prev,
        ZonedDateTime::of(LocalDateTime::of(2019, 12, 31, 23, 59, 59), ZoneId::UTC).with_nano(999_999_999)
    );
}
...
fn with_setters_for_both_date_and_time_parts_for_zoned() {
    ...
    let zdt2 = zdt.with_nano(123_000_000);
    assert_eq!(zdt2.inner().time().nanosecond(), 123_000_000);
}

#[test]
fn conversions_to_local_types_for_zoned() {
    let zdt = ZonedDateTime::of(LocalDateTime::of(2023, 5, 6, 1, 2, 3), ZoneId::UTC).with_nano(987_654_321);
