...
fn now_with_clock_matches_instant_conversions_utc() {
    ...
    assert_eq!(lt_now.seconds(), ref_lt.seconds());
    ...
    assert_eq!(ldt_now.day(), ref_ldt.day());
    ...
    assert_eq!(zdt_now.inner().unix_timestamp(), ref_zdt.inner().unix_timestamp());
    assert_eq!(odt_now.inner().unix_timestamp(), ref_odt.inner().unix_timestamp());
}
...
fn now_with_clock_respects_with_zone_on_clock_placeholder_semantics() {
    ...
    let clock_ny = clock_utc.with_zone(ZoneId::of("America/New_York"));
    ...
    assert_eq!(zdt_utc.inner().unix_timestamp(), zdt_ny.inner().unix_timestamp());

    // And they each match the underlying instant-conversion at the epoch-second level
    assert_eq!(zdt_utc.inner().unix_timestamp(), base.at_zone(ZoneId::UTC).inner().unix_timestamp());
    assert_eq!(zdt_ny.inner().unix_timestamp(), base.at_zone(ZoneId::of("America/New_York")).inner().unix_timestamp());
}
